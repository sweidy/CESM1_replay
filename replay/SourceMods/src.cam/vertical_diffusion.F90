...
contains
  ...
  subroutine vd_readnl(nlfile)
    ...
    namelist /vert_diff_nl/ kv_top_pressure, kv_top_scale, kv_freetrop_scale, eddy_lbulk_max, eddy_leng_max, eddy_max_bot_pressure, &
                            diff_cnsrv_mass_check, do_iss
    !-----------------------------------------------------------------------------
  ...
  subroutine vertical_diffusion_init(pbuf2d)
    ...
    if( do_molec_diff ) then
       ...
       if (errstring /= '') call endrun('vertical_diffusion_init: error return from init_molec_diff: '&
            //trim(errstring))
   ...
   if (masterproc) write(iulog, fmt='(a,i3,5x,a,i3)') 'NTOP_EDDY  =', ntop_eddy, 'NBOT_EDDY  =', nbot_eddy
    ...
    if( do_tms ) then
       ...
       if (errstring /= '') call endrun('vertical_diffusion_init: error return from init_tms: '&
            //trim(errstring))
    ...
    if( errstring /= '' ) call endrun( errstring )
             ...
             col_loop: do i = 1, ncol
                ...
                if (sum1>1.e-36_r8) then
                   if( abs((sum2-sum1)/sum1) .gt. 1.e-12_r8  ) then
                      write(iulog,'(a,a8,a,I4,2f8.3,5e25.16)') 'MASSCHECK vert diff : nstep,lon,lat,mass1,mass2,sum3,sflx,rel-diff : ', &
                           trim(cnst_name(m)), ' : ', nstep, state%lon(i)*180._r8/pi, state%lat(i)*180._r8/pi, sum1, sum2, sum3, sflx, abs(sum2-sum1)/sum1
