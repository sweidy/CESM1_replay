...
! not intent(inout). +++mhwang 
         real(r8), intent(in) :: tl(plev) ! Global grid temperature (K)                 ! state%t
         real(r8), intent(in) :: ql(plev) ! Global grid water vapor (g/g)               ! state%q(1)
...
!  Output
         ...
         real(r8), intent(out) :: ultend(plev) ! tendency of ul                ! tendencies are output 
...
!---mhwang
 ...
 ! -dschanen UWM 11 Feb 2010
          ...
          qtend(k) = (ql(l)+qccl(l)+qiil(l)-q0(k))*idt_gl ! where relaxation to large scale happens
...
! test water conservtion problem
      do k=1, nzm
        ...
        if(abs(qtot(9)-qtot(1))/qtot(1).gt.1.0e-5) then
