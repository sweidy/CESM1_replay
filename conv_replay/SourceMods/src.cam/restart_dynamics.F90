...
!-----------------------------------------------------------------------
  ...
  use pio,            only: file_desc_t, io_desc_t, var_desc_t, &
                             ...
                             pio_initdecomp, pio_freedecomp,  pio_closefile
...
!       deallocate(restartvars(i)%vdesc)
!       allocate(restartvars(i)%vdesc)
    end if
...
enddo
    ...
    deallocate(ldof)
        ...
        ldof => get_restart_decomp(plon, plat, plev) !added
        call pio_initdecomp(pio_subsystem, pio_double, (/plon, plat, plev/), ldof, iodesc3d) !added
        deallocate(ldof) !added
    do i=1,restartvarcnt
       call get_restart_var(i, name, timelevels, ndims, vdesc)
