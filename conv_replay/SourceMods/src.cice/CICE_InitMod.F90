...
! !INTERFACE:
!
      subroutine CICE_Init( mpicom_ice, passed_ymd,passed_tod )
!
! !USES:
!
!
...
!
      ...
      integer,optional, intent(in) :: passed_ymd
      integer,optional, intent(in) :: passed_tod
!EOP
!
!     local temporary variables
      ...
      integer  (kind=int_kind)     :: ymd,tod,yr,mon,day
      logical, save                :: first_time=.true.
!      character(len=*)             :: ice_ic
...
if (.not. present(passed_ymd)) then
      call input_data
      ...
      call init_evp (dt_dyn)
      call init_coupler_flux    ! initialize fluxes exchanged with
      call init_thermo_vertical ! initialize vertical thermodynamics
      call init_itd             ! initialize ice thickness distribution
      call init_state           ! initialize the ice state
      call init_transport       ! initialize horizontal transport
        yr=passed_ymd/10000
        mon=(passed_ymd-yr*10000)/100
        day=passed_ymd-yr*10000-mon*100

        write(ice_ic,'(".cice.r.",I4.4,"-",I2.2,"-",I2.2,"-",I5.5,".nc")') yr,mon,day,passed_tod-10800
        fname  = trim(runid) // ice_ic

        if (first_time) then
            first_time=.false.
        else
        call restartfile (usepio=.true., ice_ic=fname)
        end if
        call init_shortwave
    endif
