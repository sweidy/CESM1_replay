...
! !USES:
    ...
    use ice_age, only: write_restart_age, init_age
    use ice_meltpond, only: write_restart_pond
    use ice_FY, only: write_restart_FY, init_fy
    use ice_lvl, only: write_restart_lvl, init_lvl
...
!    call seq_cdata_setptrs(cdata_i, ID=ICEID, mpicom=mpicom_loc, &
!         gsMap=gsMap_ice, dom=dom_i, infodata=infodata)
!    call cice_init( mpicom_loc, passed_ymd=curr_ymd,passed_tod=curr_tod)
...
!
  ...
  if ( year > 99999   ) then
     ...
     write(rdate,'(i4.4,"-",i2.2,"-",i2.2,"-",i5.5)') year,month,day,ncsec
  end if
