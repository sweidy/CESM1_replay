...
!#######################################################################
   ...
   subroutine cam_read_restart( cam_out, dyn_in, dyn_out, pbuf2d, stop_ymd, stop_tod, NLFileName, passed_loc)
...
!-----------------------------------------------------------------------
     use physics_buffer, only: physics_buffer_desc
      ...
      use camsrfexch,       only: atm2hub_alloc, atm2hub_deallocate
#if (defined SPMD)
...
if ( .not. present(passed_loc) ) then
   ...
   call cam_pio_openfile(File, trim(locfn), 0)
    ...
    call cam_pio_openfile(File, trim(passed_loc), 0)
endif
...
if ( .not. present(passed_loc)) then
   ...
   call read_restart_physics( File, cam_out, pbuf2d,is_initial=.FALSE. )
endif

   if (nlres .and. .not.lbrnch .and. .not. present(passed_loc)) then
