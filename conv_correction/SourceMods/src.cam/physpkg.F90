...
subroutine phys_register
   ...
   call pbuf_add_field('T_TTEND_OLD', 'global', dtype_r8,(/pcols,pver,pbuf_times/), t_ttend_oldid)
...
! moist physics for diagnostics purposes. -Marat
    ...
    if (use_SPCAM) then
      ...
      call crm_physics_tend(ztodt, state, tend, ptend, pbuf, dlf, cam_in, cam_out) ! ptend is output
      if (masterproc) then
...
! as it was added above as part of crm tendency.
    ...
    ! this is where Ned added the convective tendency stuff (tdelta)
...
!-----------------------------determine filename--------------------------------------
   if (mon<10) then
       if (day<10) then
           write (filename, '("/n/holylfs04/LABS/kuang_lab/Lab/sweidman/IC/replay_conv_replay.","0", I1,"-0",I1,"-",I1,".nc")' ) mon, day,modstep
       else 
          write (filename,'("/n/holylfs04/LABS/kuang_lab/Lab/sweidman/IC/replay_conv_replay.","0",I1,"-",I2,"-",I1,".nc")' ) mon, day,modstep 
       endif 
    else   
       if (day<10) then
          write (filename,'("/n/holylfs04/LABS/kuang_lab/Lab/sweidman/IC/replay_conv_replay.",I2,"-0",I1,"-",I1,".nc")' ) mon, day,modstep 
       else 
          write (filename,'("/n/holylfs04/LABS/kuang_lab/Lab/sweidman/IC/replay_conv_replay.",I2,"-",I2,"-",I1,".nc")' ) mon, day,modstep 
       endif 
    endif  
...
!modstep=mod(modstep+1,8)

!end do

!-------------------------------------------------------------------------------------
if (masterproc) print*, "IC filename = ", filename    ! print filename used 
  ...
  if (modstep6hr==0 .AND. fileexists) then 
   ...
   call pio_closefile(File)  
        ...
        do c = begchunk, endchunk
            ...
            do i = 1, ncols
                do k=1,pver
                    state(c)%qforce(i,k)=Qfield3d(i,k,c)
                    state(c)%uforce(i,k)=Ufield3d(i,k,c)
                    state(c)%vforce(i,k)=Vfield3d(i,k,c)
                    state(c)%sforce(i,k)=Tfield3d(i,k,c)
                end do
            end do
        end do
...
print*, state(begchunk)%sforce(1,2)
endif
...
if (modstep6hr==0 .AND. fileexists) then 
   ...
   call pio_closefile(File)  
        ...
        do c = begchunk, endchunk
            ...
            do i = 1, ncols
                do k=1,pver
                    state(c)%qconvforce(i,k)=(Qfield3d(i,k,c))/conv_forcingtime
                    state(c)%uconvforce(i,k)=(Ufield3d(i,k,c))/conv_forcingtime
                    state(c)%vconvforce(i,k)=(Vfield3d(i,k,c))/conv_forcingtime
                    state(c)%sconvforce(i,k)=(Tfield3d(i,k,c))/conv_forcingtime
                end do
            end do
        end do
