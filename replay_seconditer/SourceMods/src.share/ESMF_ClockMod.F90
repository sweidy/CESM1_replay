...
!EOP
      ...
      IF ( PRESENT(ContinueTime) ) clock%clockint%ContinueTime = ContinueTime
...
!  integer(ESMF_KIND_I8) :: advance_count
! integer         :: ncsec
...
!integer,save :: prev_tod=-1
integer :: did_restart,count_restart
integer :: ocean_count
integer :: start_y, start_m, start_d

!  type(ESMF_Time) :: StartTime            ! start date for run
!  type(ESMF_Time) :: Stoptime            ! stop date for run
  type(ESMF_Time) :: CurrTime             ! temporary date used in logic
!  type(ESMF_Time) :: RefTime              ! reference date for time coordinate
!  type(ESMF_TimeInterval) :: TimeStep     ! timestep size
 ! integer(ESMF_KIND_I8) :: AdvanceCount
...
if (tod==0 .AND. day==1 .AND. mon==1) print*,yr,mon,day
...
!    call ESMF_TimeSet(CurrTime, yy=yr, mm=mon, dd=day, s=tod )
   CurrTime = clock%clockint%CurrTime
   ...
   call ESMF_TimeGet(CurrTime, yy=yr, mm=mon, dd=day, s=tod )
    ...
    ! call get_curr_date(yr=yr, mon=mon, day=day, ncsec=ncsec)


     !stepp= 86400*(day-1) + tod/1800
...
!if (mod(tod,21600) .ge. 10800) then
!    did_restart=1
!else if (mod(tod,21600) < 5400 .and. do_restart) then
!    did_restart=1
!else if (mod(tod,21600) .ge. 5400 .and. .not. do_restart) then
!    did_restart=1
!else
!    did_restart=0
!end if
          ...
          IF ( alarm%alarmint%Enabled ) THEN
            IF ( alarm%alarmint%RingIntervalSet ) THEN
              ...
              IF ( alarm%alarmint%StopTimeSet ) THEN
                PRED1 = AdvanceTime > alarm%alarmint%StopTime
              ENDIF
              IF ( alarm%alarmint%RingTimeSet ) THEN
