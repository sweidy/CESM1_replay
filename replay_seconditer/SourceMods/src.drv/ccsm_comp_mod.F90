...
#include <mpif.h>
   ...
   character(CL) :: rest_file,rest_file_end         ! restart file path + filename
...
subroutine ccsm_run()
 ...
 107  format( A, 2i8, A, f12.4, A, f12.4 )
   ...
   do while ( .not. stop_alarm)
      ...
      call seq_timemgr_EClockGetData( EClock_d, curr_ymd=ymd, curr_tod=tod )
      call seq_timemgr_EClockGetData( EClock_l, curr_tod=a_tod )
      call shr_cal_date2ymd(ymd,year,month,day)
      stop_alarm    = seq_timemgr_alarmIsOn(EClock_d,seq_timemgr_alarm_stop)
...
if  ( mod(tod,21600)==0) then
    ...
    !        first_time=.false.
    !    else
    !    write(rest_file_end,'(".cpl.r.",I4.4,"-",I2.2,"-",I2.2,"-",I5.5,".nc")') year,month,day,tod-10800
    !    call seq_infodata_GetData(infodata,case_name=case_name)
    !    rest_file=trim(case_name) // trim(rest_file_end)
    !    call seq_rest_read(rest_file) 
    !    end if
        do_restart=.False.
     end if
